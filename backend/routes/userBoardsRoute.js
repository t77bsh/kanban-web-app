...
userBoardsRouter.get("/user_boards/:uid", async (req, res) => {
  try {
    const requestedUserDoc = await userBoard.findOne({
...
userBoardsRouter.get("/user_boards/:uid/:boardUrlID", async (req, res) => {
  try {
    ...
    const board = requestedUserDoc.boards.find(
      (board) => board.boardUrlID === req.params.boardUrlID
    );
    console.log(board);
    res.json(board);
  } catch (err) {
...
userBoardsRouter.get("/:uid/boards/:boardUrlID", async (req, res) => {
  try {
    const requestedBoard = await userBoard.findOne(
      {
        fbaseUID: req.params.uid,
...
userBoardsRouter.patch(
  "/user_boards/update-board-name/:uid/:boardUrlID",
  async (req, res) => {
    try {
      const updatedDoc = await userBoard.findOneAndUpdate(
        {
...
userBoardsRouter.patch(
  "/user_boards/delete-board/:uid/:boardUrlID",
  async (req, res) => {
    try {
      const updatedDoc = await userBoard.findOneAndUpdate(
        { fbaseUID: req.params.uid },
...
userBoardsRouter.delete(
  "/user_boards/delete-task/:uid/:boardUrlID/:colIndex/:taskIndex",
  async (req, res) => {
    try {
      let user = await userBoard.findOne({ fbaseUID: req.params.uid });
      let board = user.boards.find(
        (board) => board.boardUrlID === req.params.boardUrlID
      );
...
userBoardsRouter.patch(
  "/user_boards/add-col/:uid/:boardUrlID",
  async (req, res) => {
    try {
      const updatedDocument = await userBoard.findOneAndUpdate(
        ...
        {
          new: true,
          arrayFilters: [{ "board.boardUrlID": req.params.boardUrlID }],
        }
      );
      res.json(
        updatedDocument.boards.find(
          (board) => board.boardUrlID === req.params.boardUrlID
        ).cols
      );
    } catch (err) {
...
userBoardsRouter.patch(
  "/user_boards/update-col-name/:uid/:boardUrlID/:colID",
  async (req, res) => {
    try {
...
userBoardsRouter.patch(
  "/user_boards/delete-col/:uid/:boardUrlID/:colID",
  async (req, res) => {
    try {
      await userBoard.findOneAndUpdate(
        { fbaseUID: req.params.uid },
...
userBoardsRouter.patch(
  "/user_boards/update-cols/:uid/:boardUrlID",
  async (req, res) => {
    try {
      const updatedDocument = await userBoard.findOneAndUpdate(
        {
...
userBoardsRouter.patch(
  "/user_boards/add-task/:uid/:boardUrlID/:colID",
  async (req, res) => {
    try {
      const updatedDocument = await userBoard.findOneAndUpdate(
        {
...
userBoardsRouter.patch(
  "/user_boards/update-task/:uid/:boardUrlID/:colID/:taskIdx",
  async (req, res) => {
    try {
      const updatedDocument = await userBoard.findOneAndUpdate(
        { fbaseUID: req.params.uid },
        {
          $set: {
            [`boards.$[board].cols.$[col].tasks.${req.params.taskIdx}`]:
              req.body,
          },
        },
        {
